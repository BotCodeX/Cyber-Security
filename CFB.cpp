...
vector<string> Encryption(string plain)
{
    ...
    string IVupdate = inivector;

    for(int i=0 ; i<blocks.size() ; i++)
    {
        string encr = DES(IVupdate);
        string cipher = xorString(encr,blocks[i]);
...
string Decryption(vector<string> cipher)
{
    ...
    {
        string decr = DES(IVupdate);
        string ans = xorString(decr,cipher[i]);
        text.push_back(ans);
        IVupdate=cipher[i];
    }
...
int main()
{
    ...
    cout << "The text was: " << retrive<<endl;
    return 0;
}
